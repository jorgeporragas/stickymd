...
| `Toggle` | A setting that is on or off. The knob is a `.lozenge` at the same diameter as a window control, so a setting reads as something you press rather than a form control that wandered in; on is `--signal-engaged`, which is what a held setting looks like everywhere else. Takes `on`, `label`, `onChange`. | The knob moves rather than fades. It is an object, and objects travel. |
