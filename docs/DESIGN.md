...
| `--bezel` | The seat under a tint swatch: a bright hairline along its top inside edge, a soft shade along its bottom |
| `--swatch-rim` | A swatch's edge — its own hue darkened, via `currentColor`, never a neutral border |
