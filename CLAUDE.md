...
- Write the sidecar index through the lock and the temporary-file rename in `index.rs`. Never write it in place, and never read-modify-write it without holding the lock — two windows doing that at once lose one of the two changes.
